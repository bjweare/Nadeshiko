...
#  “libopus” – best, but only for webm, hence libvpx-vp9.
...
#
 # Subtitles and audio
#  By default, hardcode subtitles into video and encode audio.
#
subs=yes
audio=yes
#
...
#
#  Profiles imply 16:9 aspect ratio, for a nonstandard resolution, a higher
#    profile will be taken and its bitrate “tailored” according to resolution.
#  Desired bitrates are set separately for libx264 and libvpx-vp9, as VP9
#    needs only 0.6–0.7 of libx264 bitrate for preserving the same quality.
#  VP9 also needs quantiser thresholds, which are set with min_q and max_q
#    respectively (-crf isn’t and shouldn’t be set). Setting libvpx_min_q
#    or libvpx_max_q below cancels the resolution profile values here.
#  min_q values are commented, for the longer is the duration and the lower
#    is the resolution, the stronger bitrate undershooting strikes,
#    when min_q is used in addition to max_q to constrain it (or whip it up).
#
bitres_profile_360p=(
...
libx264_pass2_extra_options=()
 ...
 # libvpx-vp9 options, that are unsafe to change
#
...
#libvpx_max_q=23
 ...
 # libvpx-vp9 options, that are safe to change
#
#  Tile columns
...
#    encoding modules for Nadeshiko (custom AC1, anyone?).
